[Mesh]
  ...
  [gen]
    ...
    dim = 3
    xmin = 0
    xmax = 17e-3     # 1  17e-3  17mm
    ymin = 0
    ymax = 10e-3     # 0.5  10e-3  10mm
    zmin = 0
    zmax = 5e-3      # 0.1  5e-3  5mm
    nx = 100      # 10   100
    ny = 80       # 5    80  20
    nz = 160       # 1    40  80  120
  []
...
[ICs]
  [temp_substrate]
    type = ConstantIC
    variable = temp
    value = 1300   # 300, preheat 1025 C ~= 1300 K
  []
[]
...
[Materials]
  ...
  [volumetric_heat]
    ...
    rx = 0.000375 # 0.000375   Ding2019, Figure 12, width ~= 520 mum => r = 0.5 mm = 0.0005  0.000375m = 0.375mm
    ry = 0.000375 # 0.000375
    rz = 0.000375 # 0.000375
    power = 400 # 300 60 65 300 400  600
    efficiency = 0.9 # 0.3  0.9
    factor = 3   # 2 20 10  2 4 3
    function_x = path_x
    function_y = path_y
    function_z = path_z
  []
  ...
  [density]
    ...
    prop_names = 'density'
    prop_values = 7609 # kg/m^3 = 1e-9 kg/mm^3
  []
  [heat]
    type = ADHeatConductionMaterial
    specific_heat_temperature_function = 500 #  J/kg·K
    thermal_conductivity_temperature_function = 25 # W/m·K  25 W/m·K = 25e3 W/mm·K
    temp = temp
  []
  ...
  [Undercooling_M]
    ...
    constant_names = 'T_m'
    constant_expressions = '1610'    # '1620' for 316L Stainless Steel, '1610K = 1336.00C' for IN718
    expression = "T_m - temp"
    outputs = exodus
  []
  [Dendrite_Growth_Rate_M]
    ...
    constant_expressions = '1610 0.00001 1.00'  # '1620.00' for 316L Stainless Steel, '1610K = 1336.00C' for IN718
  ...
  [solidification_rate]
    ...
    solidus_temperature = 1528.00   # '1531.50' for 316L Stainless Steel, '1528K = 1260.00C' for IN718
    liquidus_temperature = 1610.00  # '1620.00' for 316L Stainless Steel, '1610K = 1336.00C' for IN718
    temperature = temp
    outputs = exodus
  []
[]
...
[Functions]
  [path_x]
    type = ParsedFunction
    expression = 300e-3*t # 2*cos(2.0*pi*t)  8.47e-3*t 100e-3*t 1000e-3*t  300e-3*t  300*tmm
  []
  [path_y]
    type = ParsedFunction
    expression = 5e-3 # 2*sin(2.0*pi*t)   0 0.0012  5e-3 5mm
  []
  [path_z]
    type = ParsedFunction
    expression = 5e-3 # 1 0.001 0.0012 0.0008  0.001  5e-3 5mm
  []
...
[VectorPostprocessors]
  [point_value_vector_postprocessor_u]
    ...
    samples_file = data_points_11_SV_FinerZ.csv      #  only for modified code for reading CSV File      data_points_test.csv  data_points_solid1.csv 
...
[Executioner]
  ...
  start_time = 0.0
  end_time = 0.056 # 20  0.5 0.2 0.17 0.017 0.056
...
[Outputs]
  ...
  file_base = 'outputs/SimplerVerification/Test_7_400W_5_MPCorrected_TSSmall4X_zF4X_factor3_ybk_hypre/Test_7_400W_Ding2019_5_MPCorrected_timeStepSmall4X_zF4X_factor3_ybk_hypre_out'
