[Mesh]
  ...
  [gen]
    ...
    dim = 3
    xmin = 0
    xmax = 5e-3     # 1  17e-3  5e-3
    ymin = 0
    ymax = 2e-3     # 0.5  10e-3  2e-3
    zmin = 0
    zmax = 1.3e-3      # 0.1  5e-3  1.3e-3
    nx = 150      # 10   100  150
    ny = 60       # 5    80  20  60
    nz = 100       # 1    40  80  120  160
  []
[]
...
[ICs]
  [temp_substrate]
    type = ConstantIC
    variable = temp
    value = 300   # 300, preheat 1025 C ~= 1300 K
  []
[]
...
[BCs]
  ...
  # # Dirichlet BC on bottom (Testing)
  # [Dirichlet_top_test]
  #   type = DirichletBC
  #   boundary = bottom
  #   value = 300
  ...
  [radiation_top_1] # currently in use
    ...
    emissivity_function = 0.27 # '3/(5.670367e-8*4*300*300*300)'    0.20   0.8  0.27  # stefan boltzmann constant = 5.670367e-8 W/m^2K^4
...
[Materials]
  ...
  [volumetric_heat]
    ...
    rx = 0.0012 # 0.000375   Ding2019, Figure 12, width ~= 520 mum => r = 0.5 mm = 0.0005  0.000375  1.2 mm = 0.0012
    ry = 0.0012 # 0.000375
    rz = 0.0012 # 0.000375
    power = 1200 # 300 60 65 300 400  1200
    efficiency = 0.8 # 0.3  0.9   0.8
    factor = 0.5   # 2 20 10  2 4 3 1
    function_x = path_x
    function_y = path_y
    function_z = path_z
  []
  ...
  [density]
    ...
    prop_names = 'density'
    prop_values = 8000 # kg/m^3 = 1e-9 kg/mm^3   7609  8000
  []
  [heat]
    type = ADHeatConductionMaterial
    specific_heat_temperature_function = 440 #  J/kg·K  500  440
    thermal_conductivity_temperature_function = 13.58 # W/m·K  25 W/m·K  13.58
    temp = temp
  []
  ...
  [Undercooling_M]
    ...
    constant_names = 'T_m'
    constant_expressions = '1710.26'    # '1620.00  1710.26' for 316L Stainless Steel, '1610K = 1336.00C' for IN718
    expression = "T_m - temp"
    outputs = exodus
  []
  [Dendrite_Growth_Rate_M]
    ...
    constant_expressions = '1710.26 0.00001 1.00'  # '1620.00  1710.26' for 316L Stainless Steel, '1610K = 1336.00C' for IN718
  ...
  [solidification_rate]
    ...
    solidus_temperature = 1683.68   # '1531.50  1683.68' for 316L Stainless Steel, '1528K = 1260.00C' for IN718
    liquidus_temperature = 1710.26  # '1620.00  1710.26' for 316L Stainless Steel, '1610K = 1336.00C' for IN718
    temperature = temp
    outputs = exodus
  []
[]
...
[Functions]
  [path_x]
    type = ParsedFunction
    expression = 100e-3*t # 2*cos(2.0*pi*t)  8.47e-3*t 100e-3*t 1000e-3*t  300e-3*t 100e-3*t
  []
  [path_y]
    type = ParsedFunction
    expression = 1e-3 # 2*sin(2.0*pi*t)   0 0.0012  5e-3  1e-3
  []
  [path_z]
    type = ParsedFunction
    expression = 1.3e-3 # 1 0.001 0.0012 0.0008  0.001  5e-3  1.3e-3
  []
...
[VectorPostprocessors]
  [point_value_vector_postprocessor_u]
    ...
    samples_file = data_points_13_SV_Miyata.csv     #  only for modified code for reading CSV File      data_points_test.csv  data_points_solid1.csv 
...
[Executioner]
  ...
  start_time = 0.0
  end_time = 0.05 # 20  0.5 0.2 0.17 0.017 0.056
  dt = 0.001 # 1  0.01 0.001 0.004
  dtmin = 1e-4
[]
...
[Outputs]
  ...
  file_base = 'outputs/SimplerVerification/Test_8_1200W_1_factorHalf_hypre/Test_8_1200W_Miyata_1_factorHalf_hypre_out'
